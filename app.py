...
if submitted:
    ...
    eligible = int(CLASS_MODEL.predict(features)[0])
    probability = float(CLASS_MODEL.predict_proba(features)[0, 1]) if hasattr(CLASS_MODEL, "predict_proba") else None
    max_emi = float(REG_MODEL.predict(features)[0])
